...
public class R2dbcConfig {
    ...
    public TransactionalOperator userTransactionalOperator(
            @Qualifier("userTransactionManager") ReactiveTransactionManager userTransactionManager) {
        return TransactionalOperator.create(userTransactionManager);
    }

    @Bean
    public TransactionalOperator locationTransactionalOperator(
            @Qualifier("locationTransactionManager") ReactiveTransactionManager locationTransactionManager) {
    ...
    @Bean
    @Primary
    public ConnectionFactory userConnectionFactory(@Qualifier("userR2dbcProperties") R2dbcProperties userProperties) {
