...
public class InputValidatorServiceImpl implements InputValidatorService {
    ...
    private Mono<List<String>> validateLocation(String country, String state, String city, String postalCode) {
        List<String> locationErrors = new ArrayList<>();

        return countryRepository.findByIso3(country)
                .flatMap(foundCountry -> {
                    if (state == null || state.isEmpty()) {
                        return handleStateEmpty(foundCountry.getId(), city, postalCode, locationErrors);
                    }
                    return handleStatePresent(foundCountry.getId(), state, city, postalCode, locationErrors);
                })
    ...
    private Mono<List<String>> handleStateEmpty(Integer countryId, String city, String postalCode,
        ...
        return stateRepository.findByNameAndCountryId(city, countryId)
                .flatMap(foundState -> processCityAndPostalCode(city, foundState.getId(), postalCode, locationErrors))
                .switchIfEmpty(Mono.defer(() -> cityRepository.findByName(city)
                        .flatMap(foundCity -> processPostalCodeByCity(postalCode, foundCity.getId(), locationErrors))
                        .switchIfEmpty(handleCityNotFound(city, postalCode, locationErrors))));
    }

    private Mono<List<String>> handleStatePresent(Integer countryId, String state, String city, String postalCode,
            List<String> locationErrors) {
        return stateRepository.findByNameAndCountryId(state, countryId)
                .flatMap(foundState -> processCityAndPostalCode(city, foundState.getId(), postalCode, locationErrors))
                .switchIfEmpty(Mono.defer(() -> handleStateNotFound(state, city, postalCode, locationErrors)));
    }

    private Mono<List<String>> processCityAndPostalCode(String city, Integer stateId, String postalCode,
            List<String> locationErrors) {
        return cityRepository.findByNameAndStateId(city, stateId)
                .flatMap(foundCity -> postalCodeRepository.findByPostalCodeAndStateId(postalCode, stateId)
                        .flatMap(foundPostalCode -> Mono.just(locationErrors))
                        .switchIfEmpty(Mono.defer(() -> handlePostalCodeNotFound(postalCode, locationErrors))))
                .switchIfEmpty(Mono.defer(() -> handleCityNotFound(city, postalCode, locationErrors)));
    }

    private Mono<List<String>> processPostalCodeByCity(String postalCode, Integer cityId, List<String> locationErrors) {
        return postalCodeRepository.findByPostalCodeAndCityId(postalCode, cityId)
                .flatMap(foundPostalCode -> Mono.just(locationErrors))
                .switchIfEmpty(Mono.defer(() -> handlePostalCodeNotFound(postalCode, locationErrors)));
    }

    private Mono<List<String>> handleCityNotFound(String city, String postalCode, List<String> locationErrors) {
    ...
    private Mono<List<String>> handlePostalCodeNotFound(String postalCode, List<String> locationErrors) {
